  ...
  override fun saveAll(commits: List<CommitAggregate>) {
    jdbi.withHandle<Unit, Exception> { handle ->
      ...
      val batch = handle.prepareBatch("""
        INSERT INTO $TABLE_NAME (group_id, project_path, commit_sha, author_email, commit_time, size, title, message, files_changed, test_classification, gitlab_issues_referenced, is_merge_commit, co_author) 
        VALUES (:group_id, :project_path, :commit_sha, :author_email, :commit_time, :size, :title, :message, :files_changed, :test_classification, :gitlab_issues_referenced, :is_merge_commit, :co_author)
        ON CONFLICT (group_id, project_path, commit_sha, author_email) DO UPDATE
        SET commit_time=:commit_time, size=:size, title=:title, message=:message, files_changed=:files_changed, test_classification=:test_classification, gitlab_issues_referenced=:gitlab_issues_referenced, is_merge_commit=:is_merge_commit, co_author=:co_author
      """.trimIndent())
      ...
      commits.forEach {
        batch
          ...
          .bind("co_author", objectMapper.writeValueAsString(it.coAuthor))
          .add()
      }
